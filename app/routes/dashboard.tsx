...
export default function Dashboard() {
  const { auth, kv, isLoading } = usePuterStore(); // Vérifie l'état de chargement et l'authentification
  ...
  useEffect(() => {
    if (!isLoading && !auth.isAuthenticated) {
      navigate("/?next=/dashboard"); // Si pas authentification on affiche la page login
    }
  }, [isLoading, auth.isAuthenticated, navigate]);

  useEffect(() => {
    if (!auth.isAuthenticated) return; // Ne pas charger les CV si pas authentifié
